...
const AgentTypeLLM AgentType = "llm"
...
//
// The agent can be configured to use any LLM backend by specifying the model name:
...
func NewLLMAgent(config AgentConfig) (*LLMAgent, error) {
	...
	// Create LLM client with model inference
	client, err := llm.NewClientWithModel(config.Model)
	if err != nil {
		return nil, fmt.Errorf("failed to create LLM client: %w", err)
	}

	// Initialize session for cost tracking
	session := NewSession(config.SessionID, config.ProjectName)
...
func (la *LLMAgent) GetType() AgentType {
	return AgentTypeLLM
}

// GetConfig returns the agent configuration.
func (la *LLMAgent) GetConfig() AgentConfig {
	return la.config
}

// ListModels returns available models for all LLM backends.
func (la *LLMAgent) ListModels(ctx context.Context) ([]Model, error) {
	// Return models from the LLM cost registry
	models := []Model{
		// OpenAI
...
func (la *LLMAgent) ExecutePrompt(ctx context.Context, request PromptRequest) (*PromptResponse, error) {
	startTime := time.Now()
		...
		Costs: Costs{
			{
				...
				InputCost:    resp.CostInfo.Cost * float64(resp.CostInfo.InputTokens) / float64(resp.CostInfo.InputTokens+resp.CostInfo.OutputTokens),
				OutputCost:   resp.CostInfo.Cost * float64(resp.CostInfo.OutputTokens) / float64(resp.CostInfo.InputTokens+resp.CostInfo.OutputTokens),
			},
		},
	}
...
func (la *LLMAgent) ExecuteBatch(ctx context.Context, requests []PromptRequest) (map[string]*PromptResponse, error) {
	...
	for result := range resultsChan {
		if result.err == nil {
			...
			results[result.name] = &PromptResponse{
				Request: PromptRequest{Name: result.name},
...
func (la *LLMAgent) GetCosts() Costs {
	la.mu.Lock()
	defer la.mu.Unlock()
	return la.session.Costs
}
