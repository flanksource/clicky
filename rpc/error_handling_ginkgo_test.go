...
var _ = Describe("RPC error handling", func() {
	...
	It("documents the shared error envelope and trace header", func() {
		// The envelope is documented for the servers that answer with it: a
		// generator left on the legacy default documents the legacy body instead.
		spec := NewOpenAPIGenerator(&OpenAPIConfig{StructuredErrorResponses: true}).
			GenerateFromService(&RPCService{Operations: []RPCOperation{{
