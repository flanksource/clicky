...
func (s *SwaggerServer) handleExecuteCommand(w http.ResponseWriter, r *http.Request) {
	...
	if lookupRequested {
		...
		if !hasLookup(operation) {
			s.writeError(w, r, entity.NewStatusErrorf(
				http.StatusNotFound, "lookup_not_found", "no lookup found for %s %s", r.Method, r.URL.Path,
			))
			return
		}
	...
	if operation == nil {
		s.writeError(w, r, entity.NewStatusErrorf(
			http.StatusNotFound, "operation_not_found", "no operation found for %s %s", r.Method, r.URL.Path,
		))
		return
	}
