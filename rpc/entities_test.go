...
func TestEntitiesHandler_BulkActionPublishesItsRealRoute(t *testing.T) {
	...
	assert.Equal(t, "/api/v1/"+name+"/delete", routes["delete"].Path)
	assert.Equal(t, "/api/v1/"+name+"/archive", routes["archive"].Path)
	...
	for _, action := range []string{"archive", "delete"} {
		...
		mux.ServeHTTP(exec, httptest.NewRequest(published.Method, published.Path, strings.NewReader(`{"args":["abc","def"]}`)))
