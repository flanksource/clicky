...
func TestMemThreadStoreAddUsageAccumulates(t *testing.T) {
	...
	if _, err := store.AddUsage(ctx, th.ID, TurnUsage{InputTokens: 100, OutputTokens: 40, CostUSD: 0.5}); err != nil {
		t.Fatalf("AddUsage 1: %v", err)
	}
	got, err := store.AddUsage(ctx, th.ID, TurnUsage{InputTokens: 250, OutputTokens: 60, CostUSD: 1.25})
	if err != nil {
