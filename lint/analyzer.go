...
var Analyzer = &analysis.Analyzer{
	Name:     "clickylint",
	Doc:      "detects bad usage patterns of the clicky text API",
	Run:      run,
...
func checkCompositeLiteral(pass *analysis.Pass, lit *ast.CompositeLit) {
	if !isClickyTextType(pass, lit) {
		return
	}

	// Rule 1: non-empty struct literal
	if len(lit.Elts) > 0 {
		pass.Reportf(lit.Pos(),
			"avoid direct api.Text struct literal; use NewText().Build() or api.Text{}.Append(...)")
	}
...
func checkFuncReturnType(pass *analysis.Pass, fn *ast.FuncDecl) {
	...
	name := fn.Name.Name
	if name == "Pretty" || name == "PrettyFull" || name == "PrettyRow" {
		return
	}
